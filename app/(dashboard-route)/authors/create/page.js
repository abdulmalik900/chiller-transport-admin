...
import { createAuthor, uploadImage } from '@/app/services/apiService';
...
export default function CreateAuthorPage() {
  ...
  const onSubmit = async (data) => {
    try {
      setError(null);
      console.log('Starting author creation with data:', {
        name: data.name,
        email: data.email,
      ...
      if (imageFile) {
        ...
        console.log('Image upload successful, URL:', imgUrl);
      }
      
      // Create author with API
      const authorData = {
        ...data,
        avatarUrl: imgUrl // Keep the avatarUrl
      };
      
      console.log('Sending author data to API:', {
      ...
      // Navigate to authors page after successful creation
      router.push('/authors');
    } catch (err) {
    ...
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      <div className="flex justify-between items-center mb-6">
        <div className="flex items-center">
          ...
          <h1 className="text-2xl font-bold text-gray-800">Create New Author</h1>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        ...
        <div className="px-6 py-4 bg-gray-50 border-t border-gray-200 flex flex-col sm:flex-row sm:justify-end gap-3">
          <Link
            href="/authors"
            className="w-full sm:w-auto px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 flex justify-center items-center"
          >
            Cancel
          </Link>
          <button
            ...
            className="w-full sm:w-auto inline-flex justify-center items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:opacity-50 transition-colors"
          >
            <ArrowDownTrayIcon className="mr-2 h-4 w-4" />
            {isSubmitting || uploadingImage ? 'Saving...' : 'Save Author'}
          </button>
        </div>
      </form>
    </div>
